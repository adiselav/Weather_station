...
// Otherwise use DB_NAME from env or default to 'meteo_station'
const urlDbName = MONGO_URL ? MONGO_URL.split('?')[0].split('/').pop() : null;
const DB_NAME = urlDbName && urlDbName.length > 0 && !urlDbName.includes('.') 
  ? urlDbName 
  : (process.env.DB_NAME || 'meteo_station');
